...
- [x] **速度响应镜头**（设计稿能力 5）：v1.2 已落地——scroll velocity 喂拖影/拉丝 uniform，量程校准 0.55→2.0（日常快滚可见，commit 1f5aabb/6e8eb69）。
- [ ] **v1.2 走查校准项**（环境阻塞）：2026-07-24 走查因远程 DB（104.248.226.34:8888）TCP 直连超时无法起 dev server，用户决策先 push 后补。走查要点：landscape 真 GPU 画面（墨坠成山/谷中/雾出三态）、macro 浮雕与迎光、展墙墨池倒影对位+焦散、光标拨墨（fluid 同侧）、熔断触发后 DOM 镜像恢复；scrollVel 量程、浮雕参数（scale 6.0/spec 0.12/glow 0.3）、landscape 雾密度均按估算定值，待真实设备走查后微调。

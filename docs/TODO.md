...
- [x] **v1.2 浏览器走查（2026-07-24 playwright）**：首页分段截图走查完成——序幕/macro/dive 五段（墨坠成山/谷中/雾出）/展墙墨池/终幕全部就位；墨池两处缺陷（焦散烧白、水面破碎）当场修复（e344a3e）。首页缓存兜底顺带实证：DB 不可达时页面以默认值 200 渲染。剩余真实设备校准：scrollVel 量程、浮雕参数（scale 6.0/spec 0.12/glow 0.3）、墨池水色、landscape 雾密度、指针涟漪幅度，待真机微调。

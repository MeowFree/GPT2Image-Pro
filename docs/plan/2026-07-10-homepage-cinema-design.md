...
### v1.0.2 谷段跟随化收尾（2026-07-12，已实施落地）

用户指令：继续加强，将剩下的部分也加长为动画、拉长剧情。全页
清点后仍是"普通卡片/静态出现"的残余：积分包白圆角卡、计费规则
灰底框、三折的静态标题区。全部纳入跟随语言，谷段自廊道落幕到
FAQ 之间不再有死区。

1. **InkReveal 通用滚动显影**（ink-reveal.tsx）：元素级 useScroll
   窗口显影（y+opacity，spring 平滑、滚回倒放），phase 相位错落
   （同容器元素逐个跟随）、tilt 落定微倾（纸片落案感）；
   InkRevealBoundary 供长区块一次判定 mounted/减动效；InkRule
   为自中心向两侧生长的装裱横线。transform 与 opacity 分层绑定。
2. **积分包「墨笺落案」**：白圆角卡改墨笺形制（rounded-none +
   顶部墨标条），逐张相位落案（列内 0.18 相位差 + 交替 ±1.1 度
   微倾），滚回收起；交互不动。
3. **计费规则「例言」**：灰底框退役，改书页凡例体——标题居中
   两侧短线、上下细线界段、条目衬线序号（一/二/…，与 FAQ 册页
   同式，folioNumeral 抽 folio-numeral.ts 共享）替换 Check 图标，
   六条 0.14 相位差逐条显影。
4. **三折标题区揭幕**：Pricing/FAQ 眉标-标题-副标三行错落显影
   （0/0.22/0.42 相位），Pricing 标题下 InkRule 装裱横档；SLA
   左栏三行同族（大数字保留自己的落墨尾段显影）。
5. **InkThread 刻度窗口化**：新增 labelFadeAt 参数——sticky 驻留
   的标签在长区块尾部会压住内容（Pricing 段积分包/例言进屏，
   走查实证），刻度改为"进段打点"语义（0.38 亮起，至淡出点收），
   Pricing 传 0.6（廊道落幕即收）。
